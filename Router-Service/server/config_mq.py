...
def config_rabbitmq(config_list):
    ...
    for line in f.readlines():
        if line[:1] != '#' and line.strip() != '':
            ...
            if item.strip() == 'CONTROL_PROTOCOL':
                CONFIG.update( { 'PROTOCOL': value.strip() } )
    f.close()
    ...
    for config_type in config_list:
        ...
        if config_type == 'CLIENT' and CONFIG.get('CLIENT', None) == None:
            return False
        if config_type == 'PROTOCOL' and CONFIG.get('PROTOCOL', None) == None:
            return False
    return True
...
def control_protocol():
    return CONFIG.get('PROTOCOL', None)
